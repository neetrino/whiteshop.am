...
export function useProductFormHandlers({
  ...
  const generateAutoSku = (baseSlug: string, index: number): string => {
    ...
    return `${normalizedBase}-${timestamp}-${index}-${randomPart}`;
  };
    ...
    try {
      ...
      if (productType === 'simple') {
        ...
        const compareAtPriceUSD = simpleProductData.compareAtPrice && simpleProductData.compareAtPrice.trim() !== ''
          ...
          : undefined;
        const enteredSku = simpleProductData.sku.trim();
        const autoGeneratedSku = generateAutoSku(currentFormData.slug || 'PROD', 1);
        const finalSimpleSku = enteredSku !== '' ? enteredSku : autoGeneratedSku;
